...
public class BinanceAggregateTradeResponseValidator {
    ...
    public RawDataValidationResult validate(BinanceAggregateTradeResponse response) {
        ...
        BigDecimal quantity = parseDecimal(response.quantity());
        if (quantity == null) {
            return RawDataValidationResult.invalid("Aggregate trade quantity is not a valid decimal value.");
        }

        if (quantity.compareTo(BigDecimal.ZERO) < 0) {
            return RawDataValidationResult.invalid("Aggregate trade quantity must be non-negative.");
        }
